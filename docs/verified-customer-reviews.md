...
- A customer can update their review; every update returns it to moderation.
...
New and updated reviews start as `pending` and remain private. A platform admin
or reviewer can approve or reject them at `/admin/customer-reviews`. Rejections
require a reason, which the customer can see on the private request page before
