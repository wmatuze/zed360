...
export async function decide(
  ...
  const parsed = businessProfileDecisionSchema.safeParse({
    decision,
  ...
  try {
    await decideProfileRevision(session.accessToken, revisionId, parsed.data);
  } catch {
    redirect("/admin/profile-revisions?result=error");
  }
